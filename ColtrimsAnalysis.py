...
def load_allhits(filename):
    ...
    dt = np.dtype([('delay', s), ('x', s), ('y', s),
               ('tof', s), ('adc1', s), ('adc2', s),
               ('index', s)])
    a = np.fromfile(filename, dtype=dt, count=-1)
    x, y, tof = a['x'], a['y'], a['tof']
    delay, adc1, adc2, index = a['delay'], a['adc1'], a['adc2'], a['index']
...
def load_2body(filename):
    ...
    dt = np.dtype([('delay', s), ('x1', s), ('y1', s),
                   ...
                   ('index', s)])
    a = np.fromfile(filename, dtype=dt, count=-1)
    x1, y1, tof1 = a['x1'], a['y1'], a['tof1']
    x2, y2, tof2 = a['x2'], a['y2'], a['tof2']
    delay, adc1, adc2 = a['delay'], a['adc1'], a['adc2']
...
def load_3body(filename):
    ...
    dt = np.dtype([('delay', s), ('x1', s), ('y1', s),
                   ...
                   ('index', s)])
    a = np.fromfile(filename, dtype=dt, count=-1)
    tof1, x1, y1 = a['tof1'], a['x1'], a['y1'] 
    tof2, x2, y2 = a['tof2'], a['x2'], a['y2'] 
    tof3, x3, y3 = a['tof3'], a['x3'], a['y3'] 
    delay, adc1, adc2 = a['delay'], a['adc1'], a['adc2']
...
def view_gate2body(xyt_list, masses, charges, p_range, offset, param_list,
    ...
    hist2d(tof1, tof2, ax, 'PIPICO Gate Preview', 'TOF 1 (ns)', 'TOF 2 (ns)', 
...
def view_gate3body(xyt_list, masses, charges, p_range, offset, param_list,
    ...
    plt.style.use('dark_background')
    fig, ax = plt.subplots(1, 1)
    hist2d(tof1, tof2+tof3, ax, 'PIPICO Gate Preview', 'TOF 1 (ns)',
...
def gate_3body(xyt_list, masses, charges, p_range, offset, param_list,
    ...
    plt.style.use('dark_background')
    fig, ax = plt.subplots(1, 1)
    hist2d(tof1, tof2+tof3, ax, '3-Body PIPICO Pre-Gate', 'TOF 1 (ns)', 
           'TOF 2 + TOF 3 (ns)', xbinsize=binsize, ybinsize=binsize)
    ax.plot(t1, poly(t1, coef), 'b')
    ax.plot(t1, poly(t1, coef) + offset, 'w')
    ax.plot(t1, poly(t1, coef) - offset, 'r')        
    ...
    fig, ax = plt.subplots(1, 1)
    hist2d(tof1, tof2+tof3, ax, '3-Body PIPICO Post-Gate', 'TOF 1 (ns)', 
           'TOF 2 + TOF 3 (ns)', xbinsize=binsize, ybinsize=binsize)
    print(len(gate2[0]), 'Ions Gated')   
    return xyt_list
